...
def consistency_loss(
    ...
    resize_factor = torch.tensor(
        [W - 1.0, H - 1.0]).cuda()[None, None, None, ...]
    ...
    pts_3d_ref = ref_depth[..., None, None] * \
        (K_inv[None, None, None] @ grid_h)
...
def infer_seq(intrinsics, img_data, disp_data, poses, mot_prob, flows, flow_masks, iijj, output_dir):
    ...
    np.savez(
        os.path.join(output_dir, f"sgd_cvd_hr.npz"),
        images=np.uint8(
            img_data_pt.cpu().numpy().transpose(0, 2, 3, 1) * 255.0),
...
if __name__ == "__main__":
    ...
    for img_path in img_paths:
        ...
        cache_dir = os.path.join(output_dir,'cache-flow')
        rootdir = os.path.join(output_dir,'reconstructions')
        print("***************************** ", scene_name)
        ...
        infer_seq(intrinsics, img_data, disp_data, poses, mot_prob, flows, flow_masks, iijj, output_dir)
