...
if __name__ == '__main__':
    ...
    for img_path in img_paths:
        seq = img_path.split('/')[-1]
        seq_save_dir = os.path.join(args.outdir, seq, 'depth-anything')
        filenames = sorted(glob.glob(os.path.join(img_path, '*.png')))
        filenames += sorted(glob.glob(os.path.join(img_path, '*.jpg')))
        print(f'\033[91mstart depth-anything on [{seq}], {len(filenames)} images in total\033[0m')
        print(f'\033[91m{seq_save_dir}\033[0m')
        final_results = []
        for filename in tqdm(filenames):
            ...
            np.save(
                os.path.join(seq_save_dir, filename.split('/')[-1][:-4] + '.npy'),
                depth_npy,
            )
