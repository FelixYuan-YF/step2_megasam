...
def image_stream(
    ...
    for t, (image_file, disp_file) in enumerate(zip(image_list, mono_disp_list)):
        ...
        depth = np.clip(
            1.0 / (1.0 / aligns[2] * (aligns[0] *
                   mono_disp + aligns[1])), 0.0, 1e4
        )
...
def save_full_reconstruction(
    ...
    np.save(
        "reconstructions/{}/intrinsics.npy".format(
            scene_name), intrinsics * 8.0
    )
...
if __name__ == "__main__":
    ...
    metric_depth_paths = sorted(
        glob.glob(
            os.path.join("%s/%s" %
                         (args.metric_depth_path, scene_name), "*.npz")
        )
    )
