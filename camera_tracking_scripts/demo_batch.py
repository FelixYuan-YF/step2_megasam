...
def save_full_reconstruction(
    ...
    np.savez(
        os.path.join(save_path,f"droid.npz"),
...
def infer_seq(image_list, mono_disp_paths, metric_depth_paths, scene_name, output_dir, args):
    ...
    #TODO: check if it is necessary to save the full reconstruction, which is not used in the visualization
...
if __name__ == "__main__":
    ...
    for img_path in img_paths:
        ...
        outdir = os.path.join(args.output_dir, scene_name)
        ...
        mono_disp_paths = sorted(
            glob.glob(
                os.path.join(outdir, 'depth-anything', "*.npy")
            )
        )
        metric_depth_paths = sorted(
            glob.glob(
                os.path.join(outdir, 'unidepth', "*.npz")
            )
        )
        infer_seq(img_path_list, mono_disp_paths, metric_depth_paths, scene_name, outdir, args)
