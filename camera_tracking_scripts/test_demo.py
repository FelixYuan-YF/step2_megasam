...
if __name__ == "__main__":
    ...
    image_list += sorted(glob.glob(os.path.join("%s" %
                         (args.datapath), "*.png")))

    # NOTE Mono is inverse depth, but metric-depth is depth!
    mono_disp_paths = sorted(
        glob.glob(
            os.path.join(args.output_dir, 'depth-anything', "*.npy")
        )
    )
    metric_depth_paths = sorted(
        glob.glob(
            os.path.join(args.output_dir, 'unidepth', "*.npz")
        )
    )
    # print(f'\033[91mmono_disp_paths: {mono_disp_paths}\033[0m')
    # print(f'\033[91mmetric_depth_paths: {metric_depth_paths}\033[0m')
